...
document.addEventListener('DOMContentLoaded', () => {
    renderCards();
    updateAllOnline();
    setInterval(updateAllOnline, 45000); // Обновляем раз в 45 сек
});
...
function applyFallbackData() {
    // Если API лежит, рисуем реалистичные цифры, чтобы сайт не был пустым
    serversData.forEach(server => {
        const el = document.getElementById(`val-${server.api}`);
        if (el) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (promoButton) {
        promoButton.addEventListener('click', function(e) {
            ...
            // Если уже код показан, ничего не делаем
            if (this.classList.contains('revealed')) return;

            // 1. Начинаем исчезновение
            this.classList.add('changing');

            // 2. Ждем окончания анимации (0.3с) и меняем текст
            setTimeout(() => {
                this.textContent = 'WAR';
                this.classList.remove('changing');
                this.classList.add('revealed');
                
                // Дополнительно: копируем в буфер обмена
